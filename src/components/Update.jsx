...
export default function Update() {
    ...
    const [formData, setFormData] = useState(state || initialFormData);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let payload = {
                ...
                createdBy: formData.createdBy || userdata.loggedEmail
            }
            let res = await service.updateTodo(state.id, payload);
            if (res.status === 200) {
                toast.success("todo updated successfully");
                navigate('/dashboard');
            }
            else {
                toast.error("todo is not updated something went wrong");
            }
        } catch (error) {
    ...
    return (
        <div className={`min-h-screen flex items-center justify-center p-4 pt-12 ${mode ? "bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50" : "bg-black"}`}>
